...
# --- Sidenav toggle (show/hide sidebar) ---
if "nav_open" not in st.session_state:
    st.session_state.nav_open = True

nav_col1, nav_col2 = st.columns([1, 9])
with nav_col1:
    st.session_state.nav_open = st.toggle("Filters", value=st.session_state.nav_open, help="Show/hide the sidebar filters")

# Apply CSS to hide sidebar when toggled off
...
with trends:
    # Ensure columns are defined for this tab (avoid linter/runtime warnings)
    ...
    # ---- Projections (single-team) ----
    st.markdown("---")
    st.subheader("Projections")
    teams_in_view = sorted([t for t in f["team"].dropna().unique()])
    if len(teams_in_view) == 1:
        team_name = teams_in_view[0]
        st.caption("Pick a metric and overlay a 12-week projection (Holt linear smoothing). Double-click charts above to clear selections.")

        metric_map = {
            "Actual Output": "Actual Output",
            "Actual UPLH": "Actual UPLH",
            "Completed Hours (Actual)": "Completed Hours",
        }
        proj_metric = st.selectbox("Metric for projection", list(metric_map.keys()), index=0, key="proj_metric")
        horizon_weeks = st.slider("Horizon (weeks)", 4, 24, 12, key="proj_horizon")

        series_df = (
            f.loc[f["team"] == team_name, ["period_date", metric_map[proj_metric]]]
            .dropna()
            .sort_values("period_date")
            .rename(columns={metric_map[proj_metric]: "y"})
        )

        if series_df.shape[0] < 3:
            st.info("Not enough history to project (need ≥ 3 points).")
        else:
            # Holt's linear trend (simple implementation like your earlier button)
            y = series_df["y"].astype(float).values
            alpha = float(np.clip(2.0 / (len(y) + 1), 0.2, 0.8))
            beta = alpha / 2.0
            level, trend = y[0], y[1] - y[0]
            preds_in = []
            for t in range(1, len(y)):
                preds_in.append(level + trend)
                prev_level = level
            ...
            # future index (infer weekly)
            freq = pd.infer_freq(series_df["period_date"]) or "W"
            last_date = series_df["period_date"].max()
            future_index = pd.date_range(start=last_date + pd.tseries.frequencies.to_offset(freq), periods=horizon_weeks, freq=freq)
            ...
            hist = alt.Chart(series_df).mark_line(point=True).encode(
                x=alt.X("period_date:T", title="Week"),
                y=alt.Y("y:Q", title=proj_metric),
                tooltip=["period_date:T", alt.Tooltip("y:Q", format=",.2f")],
            )
            ...
            fline = alt.Chart(forecast_df).mark_line(point=True, strokeDash=[5, 5]).encode(
                x="period_date:T",
                y="y:Q",
                color=alt.value("#111827"),
                tooltip=["period_date:T", alt.Tooltip("y:Q", format=",.2f")],
            )
            st.altair_chart((hist + band + fline).properties(height=320), use_container_width=True)
    else:
        st.info("Select a single team to enable projections.")
