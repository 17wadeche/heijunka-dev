...
def enterprise_nonwip_kpi_lookup(
    *,
    ...
    """Return the same team/week KPI fields used by the Enterprise dashboard.

    Team dashboards pass their already-selected week/person accounting data into this
    helper so the Non-WIP Overview KPI cards can look up Enterprise-equivalent values
    from one central rule set instead of carrying page-specific recalculations.
    """
    ...
    if team_name in FORTY_HOUR_TEAMS:
        ...
        # Enterprise treats one CDS contributor as a 10h/week assignment unless
        # the source accounting data has already removed them from the selected team.
